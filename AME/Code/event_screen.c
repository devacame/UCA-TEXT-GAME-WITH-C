...
void tutorial(){
	...
	puts("멈추려면 p를 눌러주세요"); 
	puts("아, 그리고 계속 왔다갔다 하고 싶으실텐데, 그러면 자신에게만 안좋을 수가 있어요(멘탈에)\n");
	return ;
}
...
void map_clear(int prev){
	...
	Sleep(400);
	printf("옵션 2: 방금 깬 맵 하드모드 클리어하기 (시간제한 있으니까 열심히 해보셈)\n");
	Sleep(400);
...
void fail(int prev){
	...
	Sleep(750);
	puts("다시 도전 한다: 1 | 일반이나 해야겠다: 2 | 다른거 하드모드 해볼래: 3 | 그냥 안할래 이딴 게임: 4");
	int input=0;
	printf(">> "); scanf("%d", &input); while ((getchar()) != '\n');
